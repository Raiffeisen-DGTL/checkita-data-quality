package ru.raiffeisen.checkita.checks.load
...
object LoadCheckEnum extends Enumeration {
  // pre
  val existLC: LCDefinition = LCDefinition("EXIST", classOf[ExistLoadCheck])
  val encodingLC: LCDefinition = LCDefinition("ENCODING", classOf[EncodingLoadCheck])
  val fileTypeLC: LCDefinition = LCDefinition("FILE_TYPE", classOf[FileTypeLoadCheck])
  // post
  val columnNumLC: LCDefinition = LCDefinition("EXACT_COLUMN_NUM", classOf[ColumnLoadCheck])
  val minColumnNumLC: LCDefinition = LCDefinition("MIN_COLUMN_NUM", classOf[MinColumnLoadCheck])
...
case class ExistLoadCheck(id: String, tipo: String, source: String, option: Any) extends LoadCheck with Logging {
  ...
  override def run(confOpt: Option[SourceConfig] = None, df: Option[DataFrame] = None)
                  (implicit fs: FileSystem, sparkSes: SparkSession, settings: DQSettings): LoadCheckResult = {
    ...
    LoadCheckResult(id, tipo, source, option.toString, status, msg)
  }
}
...
case class EncodingLoadCheck(id: String, tipo: String, source: String, option: Any) extends LoadCheck with Logging {
  ...
  override def run(confOpt: Option[SourceConfig] = None, df: Option[DataFrame] = None)
    ...
    val dfOpt: Option[DataFrame] = conf.fileType.toLowerCase match {
      ...
      case "fixed" => Try {
        sparkSes.read.option("encoding", option.toString).textFile(conf.path).toDF
      }.toOption
      case "orc" => Try(sparkSes.read.option("encoding", option.toString).orc(conf.path)).toOption
      case "parquet" => Try(sparkSes.read.option("encoding", option.toString).parquet(conf.path)).toOption
...
case class FileTypeLoadCheck(id: String, tipo: String, source: String, option: Any) extends LoadCheck with Logging {
  ...
  override def run(confOpt: Option[SourceConfig] = None, df: Option[DataFrame] = None)
    ...
    val dfOpt: Option[DataFrame] = option.toString.toLowerCase match {
      case "avro" => Try(
        sparkSes.read.format("avro").load(conf.path)
      ).toOption
      case "delimited" => Try(
        sparkSes.read
          .format("csv")
      ...
      case "fixed" => Try(sparkSes.read.textFile(conf.path).toDF).toOption
      case "orc" => Try(sparkSes.read.orc(conf.path)).toOption
      case "parquet" => Try(sparkSes.read.parquet(conf.path)).toOption
      case _ => throw new IllegalArgumentException(s"Unknown source type: $option.")
    }
