...
trait StatusableCalculator extends MetricCalculator {
  ...
  override def getStatus: CalculatorStatus.Value = status
}
...
class ComposedMetricCalculator(primitiveMetrics: Iterable[MetricResult]) extends ExprParsers2 {
   ...
   */
  def run(ex: ComposedMetric)(implicit settings: DQSettings): ComposedMetricResult = {
    val jobId = primitiveMetrics.head.jobId
    ...
    ComposedMetricResult(jobId, ex.id, ex.name, ex.description, "", formulaWithParameters, result, "")
  }
...
sealed trait ExprParsers2 extends JavaTokenParsers {
  ...
  def eval(t: Tree): Double = t match {
    ...
    case Pow(t1, t2) => Math.pow(eval(t1), eval(t2))
    case Num(t)      => t
  }
  ...
  lazy val term = power ~ rep("[*/]".r ~ power) ^^ {
    case t ~ ts =>
      ts.foldLeft(t) {
  ...
  lazy val power = factor ~ rep("['^]".r ~ factor) ^^ {
    case t ~ ts =>
      ts.foldLeft(t) {
