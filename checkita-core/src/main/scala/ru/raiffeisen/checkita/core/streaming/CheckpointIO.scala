...
object CheckpointIO {
  ...
  type CheckpointRecord = (Long, String, ProcessorBuffer)
  
  private implicit val bufferEncoder: Encoder[ProcessorBuffer] = Encoders.kryo[ProcessorBuffer]
  private implicit val checkRecordEncoder: Encoder[CheckpointRecord] =
    Encoders.tuple(Encoders.scalaLong, Encoders.STRING, bufferEncoder)
  ...
  def writeCheckpoint(buffer: ProcessorBuffer,
    ...
    val record = Seq((executionTime, jobHash, buffer))
  ...
  def readCheckpoint(checkpointDir: String, 
    ...
    spark.read.parquet(readPath).as[CheckpointRecord]
      .filter($"_2" === jobHash)
      .sort($"_1".desc)
      .take(1).head._3
  }.toOption
