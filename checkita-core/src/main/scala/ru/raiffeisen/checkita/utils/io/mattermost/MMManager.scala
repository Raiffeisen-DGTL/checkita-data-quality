...
import ru.raiffeisen.checkita.utils.io.mattermost.MMModels.{EndPoints, MMConfig, MMMessage}
 ...
 */
class MMManager(conf: MMConfig) {

  private val host = conf.host
  private val token = conf.token
   ...
   */
  private val fromUserId: String = executeGetRequest(s"$host/${EndPoints.Users}/me") match {
    case Right(data) =>
  ...
  private val channels: Map[String, String] =
    executeGetRequest(s"$host/${EndPoints.Users}/$fromUserId/channels") match {
      case Right(data) =>
  ...
  private def getUserIds(usernames: Seq[String]): Seq[String] = {
    ...
    val endPoint = s"$host/${EndPoints.Users}/usernames"
  ...
  private def getDirectChannelId(userId: String): Option[String] = {
    ...
    val endPoint = s"$host/${EndPoints.Channels}/direct"
    executePostRequest(endPoint, Seq(payload)) match {
      case Right(data) =>
  ...
  private def uploadFile(channelId: String, fileName: String, fileContent: Array[Byte]): Seq[String] = {
    ...
    val endPoint = s"$host/${EndPoints.Files}"
  ...
  private def postMessage(channelId: String,
    ...
    val endPoint = s"$host/${EndPoints.Posts}"
    executePostRequest(endPoint, Seq(payload)) match {
      case Right(_) => println(s"Message was successfully send to channelId: $channelId")
      case Left(err) => throw new HttpException(err)
    }
  }
   ...
   */
  def send(message: MMMessage): Unit = {
