...
object Casting {
  ...
  @transient private lazy val ZONE_ID = ZoneId.systemDefault()
   ...
   */
  def stringToTimestamp(str: String, dateFormat: String): Option[Long] = Try{
    val formatter = TimestampFormatter(
      format = dateFormat,
      zoneId = ZONE_ID,
      legacyFormat = SIMPLE_DATE_FORMAT,
      isParsing = true,
      forTimestampNTZ = false
    )
    formatter.parse(str)
  }.toOption
   ...
   * @note Metric calculators are not intended to work with complex data types.
   ...
   * Used in formattedDate metric to support checking not only
   * full date or time string but also a part of date or time,
   * e.g. parse year only with format string `yyyy`
   *
   * @param value      Value to cast
   * @param dateFormat Date format used for casting
   * @return Optional timestamp long or None if casting wasn't successful.
   * @note Metric calculators are not intended to work with complex data types.
   *       Therefore, only primitive types can be converted to Timestamp as well as byte arrays.
   *       Attempt to convert complex data type such as Map or StructType will return None.    
   */
  def tryToTimestamp(value: Any, dateFormat: String): Option[Long] =
    value match {
      case null => None
      case date: java.sql.Date => Some(date.toLocalDate.toEpochDay)
      case localDate: java.time.LocalDate => Some(localDate.toEpochDay)
      case timestamp: java.sql.Timestamp => Some(timestamp.getTime)
      case instant: java.time.Instant => Some(instant.toEpochMilli)
      case b: Array[Byte] => stringToTimestamp(new String(b, StandardCharsets.UTF_8), dateFormat)
      case otherValue =>
        Try(primitiveValToString(otherValue)).toOption.flatMap(stringToTimestamp(_, dateFormat))
    }
}
