...
Данный набор конфигураций применим только для потоковых приложений и определяет различные аспекты выполнения проверок
качества над потоковыми источниками. Для более подробной информации о запуске фреймворка в потоковом режиме, см. главу
[Проверка Качества Потоковых Источников Данных](../02-general-concepts/05-StreamingMode.md).

* `trigger` - Триггерный интервал: определяет временной интервал за который будут собираться микро-пакеты из 
  потокового источника данных. *Опционально, по умолчанию `10s`*.
* `window` - Оконный интервал: определяет временной интервал, соответсвующий размеру окна, в котором будут 
  аккумулированы результаты расчета метрик. Все метрики и проверки вычисляются для каждого окна, как только оно будет
  полностью сформировано (опустится ниже уровня "водяного знака"). *Опционально, по умолчанию `10m`*.
* `watermark` - Уровень "водяного знака": определяет временной интервал после которого, "опоздавшие" записи не будут
  браться в обработку. *Опционально, по умолчанию `5m`*.
* `allowEmptyWindows` - Бинарный флаг, определяющий, разрешены ли "пустые" окна (окна в которые не попало ни одной  
  строки данных). Так, в ситуациях, когда окно полностью сформировано и при этом для одного или нескольких потоковых
  источников в это окно не попало ни одной строки, то все соответствующие проверки будут опущены только в том случае,
  когда данных параметр будет иметь значение `true`. В противном случае, проверки будут выполнены и вернут ошибку с 
  сообщением вида `... metric results were not found ...`. *Опционально, по умолчанию `false`*.

> **ВАЖНО** Все временные интервалы должны быть определены как строки в формате соответствующем формату
